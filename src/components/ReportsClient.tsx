...
function paymentStatus(expected: number, paid: number): ReportRow["status"] {
...
export default function ReportsClient() {
      ...
      .map((member) => {
        ...
        const paid = contributions
          ...
          .reduce((total, contribution) => {
            return total + Number(contribution.amountPaid || 0);
          }, 0);

        const payoutReceived = recipients
          .filter(
            (recipient) =>
              recipient.month === month &&
              normalizeName(recipient.memberName) === normalizeName(member.name)
          )
          .reduce((total, recipient) => {
            return total + Number(recipient.amountGiven || 0);
          }, 0);

        const balance = Math.max(expected - paid, 0);
        const netPosition = paid - payoutReceived;
  ...
  const summary = useMemo(() => {
    return reportRows.reduce(
      (totals, row) => {
        ...
        totals.payouts += row.payoutReceived;
        totals.netRetained += row.paid - row.payoutReceived;

        if (row.status === "Paid") totals.paidMembers += 1;
        if (row.status === "Partial") totals.partialMembers += 1;
        if (row.status === "Pending") totals.pendingMembers += 1;
        if (row.status === "Overpaid") totals.overpaidMembers += 1;
  ...
  function exportCsv() {
    ...
    anchor.href = url;
    anchor.download = `jirani-monthly-report-${month}.csv`;
    anchor.click();
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-slate-600">
              Generate monthly summaries for collections, balances, payout
              recipients, and member-level status.
            </p>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <div className="rounded-3xl border border-purple-100 bg-purple-50 p-4">
          <p className="text-sm font-bold text-purple-800">Payouts</p>
          <p className="mt-1 text-xl font-black text-purple-900">
            {money(summary.payouts)}
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-1 text-xl font-black text-slate-950">
            {money(summary.netRetained)}
          </p>
        </div>

        <div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-4">
          <p className="text-sm font-bold text-emerald-800">Collection rate</p>
        ...
        <div className="rounded-3xl border border-amber-100 bg-amber-50 p-4">
          <p className="text-sm font-bold text-amber-800">Partial / Pending</p>
          <p className="mt-1 text-xl font-black text-amber-900">
            {summary.partialMembers} / {summary.pendingMembers}
          </p>
        </div>
      </section>
          ...
          <table className="min-w-[1000px] w-full border-separate border-spacing-y-2 text-left text-sm">
            <thead>
              <tr className="text-xs uppercase tracking-wide text-slate-500">
                ...
                <th className="px-3 py-2">Net position</th>
